...
char * lexer_get_id(){
	...
	{   //i++;
		...
		if(isalnum(lexer->c)|| lexer->c=='_')strcat(value, lexer_get_caracter_as_string());
...
char* read_comment_1(){
	...
	while(lexer->c != '\n' && lexer->c!=EOF){
		...
		if(lexer->c!='\n'&& lexer->c!=EOF ) strcat(comment,str);		
	}
	lexer_putback();
	//printf("%c",Putback);
	return comment;
...
char* read_comment_2(){
	...
	if(lexer->c=='[') return NULL;
	if (lexer->c==']') return comment;
	if(lexer->c == EOF) {
						lexer_putback();
		 				return NULL;
		 				}
...
Token* lexer_get_next_token(){
	...
	lexer_putback();
	char * str=calloc(1,5*sizeof(char));
	if(i>0 )return token_init(TOKEN_WHITESPACE,itoa(i, str, 10));
	...
	skip_whitespace();
		...
		if(lexer->c=='['){
			...
			else return token_init(TOKEN_ERR,"Expected ]");
		...
		if(isdigit(lexer->c)){
			...
			if(strcmp(value,"d")==0) return token_init(TOKEN_D,value);
		 	...
		 	if(strcmp(value,"tstz")==0) return token_init(TOKEN_TSTZ,value);
			if(isVCNNN(value)) return token_init(TOKEN_VCNNN,value);
		...
		if (strcmp(lexer_get_caracter_as_string(),"'")==0)
		{
			...
			if(Putback==EOF) return token_init(TOKEN_ERR,"Expected ' ");
